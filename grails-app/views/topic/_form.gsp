<%@ page import="linksharingapplication.Topic" %>
...
<div class="fieldcontain ${hasErrors(bean: topicInstance, field: 'createdBy', 'error')} required">
	...
	<g:select id="createdBy" name="createdBy.id" from="${linksharingapplication.User.list()}" optionKey="id" required="" value="${topicInstance?.createdBy?.id}" class="many-to-one"/>
...
<div class="fieldcontain ${hasErrors(bean: topicInstance, field: 'visibility', 'error')} required">
	...
	<g:select name="visibility" from="${linksharingapplication.Topic$VisibilityValue?.values()}" keys="${linksharingapplication.Topic$VisibilityValue.values()*.name()}" required="" value="${topicInstance?.visibility?.name()}" />
